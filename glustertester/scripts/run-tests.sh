...
function get_tests_command()
{
    if [ "x$input_file" != "x" ]; then
        echo $(cat ${input_file})
    else
        echo $(find ${regression_testsdir}/tests -name '*.t' | LC_COLLATE=C sort)
    fi
}
